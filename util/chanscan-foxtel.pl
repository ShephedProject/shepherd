...
my $URL_FTA = "$DATASOURCE/guide/ajax/channels/ChannelsFta.aspx?region_id=";
my $URL_PAY = "$DATASOURCE/guide/ajax/channels/ChannelsPay.aspx?region_id=";
my $REGIONIDS_PAY = { 'Foxtel' => 168 };
...
foreach my $region (sort { $a <=> $b } keys %$region_channels)
{
    ...
    {
	$url = $URL_FTA . $region;
	$url2 = $DATASOURCE2 . $region . '/0/';
    }
    elsif (defined $REGIONIDS_PAY->{$region})
    {
	$url = $URL_PAY . $REGIONIDS_PAY->{$region};
	$url2 = $DATASOURCE2 . '73/168/';
    }
    else
    {
	print "!! Unknown\n";
    ...
    my $content = &Shepherd::Common::get_url($url);
    ...
    $content = &Shepherd::Common::get_url($url2);
...
sub setup_ua
{
  ...
  &Shepherd::Common::set_default('debug', 3);
